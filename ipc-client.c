...
int ipc_open_socket(const char *socket_path) {
	...
	if ((socketfd = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
		printf("Unable to open Unix socket\n");
		exit(1);
	}
...
bool ipc_set_recv_timeout(int socketfd, struct timeval tv) {
	if (setsockopt(socketfd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) == -1) {
		printf("Failed to set ipc recv timeout\n");
		return false;
	}
	return true;
}
...
error_2:
	free(response);
error_1:
	printf("Unable to allocate memory for IPC response");
	return NULL;
}
...
char *ipc_single_command(int socketfd, uint32_t type, const char *payload, uint32_t *len) {
	...
	if (write(socketfd, data, IPC_HEADER_SIZE) == -1) {
		printf("Unable to send IPC header\n");
		exit(1);
	}

	if (write(socketfd, payload, *len) == -1) {
		printf("Unable to send IPC payload\n");
		exit(1);
	}
